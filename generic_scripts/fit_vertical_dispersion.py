...
def fit_dispersion(
    ...
    for _ in range(niter):
        ...
        stren_ksl += stren
        print(
            '     residue: {:.2f}, ksl: {:.2e} 1/m (rms)'.format(
...
def plot_dispersion_fit_scan_singular_values(
    ...
    ax2.plot(
        nsvals, betabeatx, 'o-', color='C0', label=r'$\Delta \beta_x/\beta_x$'
    )
    ax2.plot(
        nsvals, betabeaty, 'o-', color='C1', label=r'$\Delta \beta_y/\beta_y$'
    )
    ...
    fig.tight_layout()
    fig.savefig(
        'vertical_dispersion_fit_vs_singular_values.png', dpi=300, format='png'
    )
    plt.show()
...
def compare_orms(modfit, orm_meas, chidx=0, cvidx=0):
    ...
    fig.tight_layout()
    fig.savefig(
        'compare_off_diagonal_orm_bad_corresp.png', dpi=300, format='png'
    )
    plt.show()
...
def plot_diff_qs_strengths(
    ...
    modfit, *_ = fit_dispersion(
        simod, disp_mat, disp_meas, bpmidx, qsidx, setup, svals=sv1, niter=10
    )
    stren_sv1 = pa.lattice.get_attribute(modfit, 'KsL', qsidx)
    modfit, *_ = fit_dispersion(
        simod, disp_mat, disp_meas, bpmidx, qsidx, setup, svals=sv2, niter=10
    )
...
if __name__ == '__main__':
    ...
    svals = 41
    modfit, *_ = fit_dispersion(
        simod, disp_mat, disp_meas, bpmidx, qsidx, setup, svals=svals, niter=10
    )
    plot_dispersion_fit(
