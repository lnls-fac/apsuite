...
import numpy as _np
from siriuspy.devices import APU as _APU, APUFeedForward as _APUFeedForward, \
    IDCorrectors as _IDCorrectors, SOFB as _SOFB
...
class MeasAPUFFWD(_BaseClass):
    ...
    def __str__(self):
        ...
        rst += "# ===========================\n"
        line = "       ".join([
            "CH-{}[A]       CV-{}[A]".format(2 * i + 1, 2 * i + 2)
            for i in range(self._nr_corrs // 2)
        ])
        rst += "# Phase[mm] " + line + "\n"
        ...
        rst += "#\n"
        rst += (
            "# 1. This table contains correctors feedforward"
    ...
    def calculate(self):
        ...
        for i, phase in enumerate(phases):
            ...
            curr_deltas, *_ = self.calculate_at_phase(**mdatum)
            ffwd[i, :] = curr_deltas
