...
class DoBBA(_BaseClass):
    ...
    def process_data_single_bpm(
        ...
        x0 = -px[1] / px[0] / 2
        y0 = -py[1] / py[0] / 2
        stdx0 = _np.abs(x0) * _np.sqrt(
            _np.sum(_np.diag(covx)[:2] / px[:2] / px[:2])
        )
        stdy0 = _np.abs(y0) * _np.sqrt(
            _np.sum(_np.diag(covy)[:2] / py[:2] / py[:2])
        )
    ...
    def make_figure_quadfit(self, bpms=None, fname='', title=''):
        ...
        gs.update(
            left=0.1, right=0.78, bottom=0.15, top=0.9, hspace=0.5, wspace=0.35
        )
    ...
    def make_figure_compare_with_initial(
        ...
        minx = (
            _np.min(_np.hstack([x0q[iok], x0c[iok], x0q[inok], x0c[inok]]))
            * 1.1
        )
        maxx = (
            _np.max(_np.hstack([x0q[iok], x0c[iok], x0q[inok], x0c[inok]]))
            * 1.1
        )
        miny = (
            _np.min(_np.hstack([y0q[iok], y0c[iok], y0q[inok], y0c[inok]]))
            * 1.1
        )
        maxy = (
            _np.max(_np.hstack([y0q[iok], y0c[iok], y0q[inok], y0c[inok]]))
            * 1.1
        )
    ...
    def make_figure_compare_methods(
        ...
        minx = (
            _np.min(_np.hstack([x0q[iok], x0l[iok], x0q[inok], x0l[inok]]))
            * 1.1
        )
        maxx = (
            _np.max(_np.hstack([x0q[iok], x0l[iok], x0q[inok], x0l[inok]]))
            * 1.1
        )
        miny = (
            _np.min(_np.hstack([y0q[iok], y0l[iok], y0q[inok], y0l[inok]]))
            * 1.1
        )
        maxy = (
            _np.max(_np.hstack([y0q[iok], y0l[iok], y0q[inok], y0l[inok]]))
            * 1.1
        )
        minx = -1 * xlim if xlim is not None else minx
        maxx = xlim if xlim is not None else maxx
    ...
    def make_figure_compare_bbas(
        ...
        minx = miny = _np.inf
        maxx = maxy = -_np.inf
        x0li, y0li = bbalist[0].get_bba_results(method=method, error=False)
