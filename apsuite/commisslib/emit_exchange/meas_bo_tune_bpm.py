...
from siriuspy.devices import CurrInfoBO, Event, EVG, FamBPMs, RFGen, Trigger
...
class BOTunebyBPM(_BaseClass):
    ...
    *Experiment Guide Lines*
    ...
    def configure_bpms(self):
        ...
        bobpms.cmd_abort_mturn_acquisition()
        bobpms.config_mturn_acquisition(
    ...
    def get_orbit(self, injection=False, ext_trigger=False):
        ...
        bobpms.reset_mturn_initial_state()
        ...
        ret = bobpms.wait_update_mturn(timeout=prms.orbit_timeout)
        if ret:
            self.data = dict()
            raise AssertionError(
                f"Problem waiting BPMs update. Error code: {ret:f}"
            )
        orbx, orby = bobpms.get_mturn_signals()

        self.data["orbx"], self.data["orby"] = orbx, orby
        self.data["timestamp"] = _time.time()

    def load_orbit(self, data=None, orbx=None, orby=None):
        """Load orbit data into the object.

        You can pass the entire data dictionary or just the orbits. If data
        argument is provided, orbx and orby become optional
        """
        if data is not None:
            self.data = data
        if orbx is not None:
    ...
    def naff_tunes(self, dn=None, bpm_indices=None, interval=None):
        """Compute the tune evolution from the BPMs matrix.

        Use with a moving window of length dn. If dn is not passed, the tunes
        are computed using all points.
        """
    ...
    def spectrogram(
        ...
        Split in segments of length dn. You also can set a window and control
        the overlap between the segments. If more than one BPMs is used, the
        outputted spectrogram is a normalized mean of the individual BPMs
        spectrograms. This method is a adaptation of scipy.signal.spectrogram
        function, see the below link for more information:
        ...
        Args:
            ...
            interval (int, optional): Sizee of interval to consider.
                Defaults to the entire array.
            overlap (int, optional): Number of points to overlap between
                segments. Default is dn//8.
            window (str|tuple|array): Desired window to use. If window is a
                string or tuple, it is passed to `scipy.signal.get_window` to
                generate the desidered window. See the below link to consult
                the avaliable windows:
                    https://docs.scipy.org/doc/scipy/reference/generated/scipy.signal.get_window.html
                You also can pass an array describing the desirable window.
                Defaults to a Tukey window with shape parameter equal to 0.25.

        Returns:
            tune1_matrix: Spectrograms of the tunes x.
            tune2_matrix: Spectrograms of the tunes y.
    ...
    def dft(self, bpm_indices=None):
        ...
        freqs = _rfftfreq(x_beta.shape[0])
...
def _plot_heatmap(freqs, revs, tune_matrix):
    _plt.figure()
    _plt.pcolormesh(revs, freqs, tune_matrix, shading="gouraud", cmap="hot")
