...
class UtilClass:
    ...
    def get_data(bbb, acqtype):
        ...
        rawdata = acq.data_raw.reshape((-1, bbb.info.harmonic_number)).T.copy()
        ...
        return dict(
            ...
            acqtype=acqtype,
            downsample=acq.downsample,
            fb_set0=bbb.coeffs.set0,
            fb_set1=bbb.coeffs.set1,
            ...
            growth_time=acq.growthtime,
            acq_time=acq.acqtime,
            hold_time=acq.holdtime,
            post_time=acq.posttime,
        )
    ...
    def _process_data(data, params, rawdata=None):
        ...
        comp = _np.exp(
            -1j * 2 * _np.pi * (int_freq + freq[None, :]) * dts[:, None]
        )
        data_dft *= comp
...
class BbBAcqData(_BaseClass, UtilClass):
    ...
    def load_and_apply_old_data(self, fname):
        ...
        data['rawdata'] = _np.array(
            data.pop('data').reshape((-1, self.params.HARM_NUM)).T, dtype=float
        )
...
class DriveDampLParams(BbBLParams):
    ...
    def __str__(self):
        ...
        stg += stmp(
            'fitting_consider_offset',
            str(bool(self.fitting_consider_offset)),
            '(True | False)',
        )
...
class MeasDriveDamp(_ThreadBaseClass, UtilClass):
    ...
    def estimate_fitting_intervals(
        ...
        interv = super().estimate_fitting_intervals(
            infos, int_type='damp', clearance=0
        )[0]
        ...
        for mode in modes_to_measure:
            ...
            print(
                ',      '.join([
                    ...
                    for m, gt in zip([mode], grt.ravel())
                ]),
                end='',
            )
            elt -= _time.time()
...
class MeasTuneShift(_ThreadBaseClass):
    ...
    def get_data_from_plane(self, plane):
        ...
        data = {
            ...
            'spec_mag': sb_tune.spec_mag,
            'spec_freq': sb_tune.spec_freq,
            'spec_phase': sb_tune.spec_phase,
            'bunch_id': sb_tune.bunch_id,
            'fft_size': sb_tune.fft_size,
