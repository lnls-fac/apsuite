...
def si_bunch_generate(params):
    ...
    # shift model to start at ScrapH
    si_model = si_lattice_shift(params)

    return bunch, si_model


def si_lattice_shift(params):
    """."""
    lat_mod = pymodels.si.lattice
    si_lattice = lat_mod.create_lattice()
    idx = pyaccel.lattice.find_indices(
        si_lattice, 'fam_name', 'ScrapH')[0]
    si_lattice = pyaccel.lattice.shift(si_lattice, idx + 1)
...
def plot_phase_space(params, rout):
    ...
    plt.grid()
    plt.legend()
    strf = 'Phase Space right after DpKckr (nr_particles: {}, nr_turns {})'
...
def run():
    ...
    # NOTE: pos and injection angle can be varied using these
    # two parameters
    params.scrapper_h_max_delta = -4.5/1000 * 0  # [m]
    params.scrapper_h_min_delta = +4.5/1000 * 1  # [m]
    ...
    si_model[idx].hkick_polynom = -0.45/1000
