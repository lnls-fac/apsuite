...
class PosAngRespmat:
    ...
    def _calc_posang_matrices(cls, model, ch_idx, cv_idx):
        ini = ch_idx[0][0]
        mat_h_aux = _np.zeros((2, len(ch_idx)))
        for idx, corr in enumerate(ch_idx):
            pyaccel.lattice.set_attribute(
                model, 'hkick_polynom', corr, cls.KICK / 2 / len(corr)
            )
            pos1, *_ = pyaccel.tracking.line_pass(
                model[ini : len(model)], 6 * [0]
            )
            pyaccel.lattice.set_attribute(
                model, 'hkick_polynom', corr, -cls.KICK / 2 / len(corr)
            )
            pos2, *_ = pyaccel.tracking.line_pass(
                model[ini : len(model)], 6 * [0]
            )
            pyaccel.lattice.set_attribute(model, 'hkick_polynom', corr, 0)
