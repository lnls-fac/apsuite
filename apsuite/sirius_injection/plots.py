...
def plot_phase_ellipse(bunch, local_twiss, eqparams, fig=None, ax=None):
    ...
    alphax, alphay = local_twiss.alphax, local_twiss.alphay
    betax, betay = local_twiss.betax, local_twiss.betay
    emitx, emity = eqparams.emitx, eqparams.emity
    gammax = (1 + alphax**2)/betax
    gammay = (1 + alphay**2)/betay
    xb, x_b = bunch[0,:], bunch[1,:] #  The bunchs entries onlys serves to give the 
    yb, y_b = bunch[2,:], bunch[3,:] # x or y limits in the graph

    x, x_, eqx = calc_ellipse_equation(gammax,alphax,betax,emitx, xb,x_b)
    y, y_, eqy = calc_ellipse_equation(gammay,alphay, betay, emity, yb, y_b)
    ...
    if (fig is not None) & (ax is not None):
        ...
        plt.contour(x*1e3,x_*1e3,eqx,[0], linewidths = 2) # Without fig, ax arguments, plots only x,x' ellipse

def plot_phase_diagram2(bunch, local_twiss, eqparams,title="", closed_orbit=None, emmit_error=False):
    """Plots phase diagram and the invariant ellipse based in emmitance and twiss parameters"""
    
    fig,ax = plot_phase_diagram(bunch,closed_orbit,title=title)
    plot_phase_ellipse(bunch, local_twiss, eqparams, fig, ax)
    
    if emmit_error:
        u_sigma,_,_ = sigmax_error(bunch, local_twiss, eqparams, outprint=False)
        ax[0].legend(['$\sigma_x$ error = {:.2f} %'.format(u_sigma*100)], loc='center left', bbox_to_anchor=(2.06, 0.5))
...
def plot_xtrajectories(model,part_out,init_idx,final_idx,title=""):
    ...
    xparticles = part_out[0,:,:] #select rx for all particles and all elements
    plt.figure()
    plt.title(title)
