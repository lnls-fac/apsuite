...
def load_dynap_xy(path, var_plane="x"):
    ...
    turn, plane, x, y = _np.loadtxt(
        fname, skiprows=nr_header_lines, usecols=(1, 3, 5, 6), unpack=True
    )
    ...
    def fun(x):
        return _np.flipud(x.reshape((nx, ny)).T)
...
def dynap_xy(**kwargs):
    ...
    _subprocess.call(args)  # noqa: S603
...
def load_dynap_ex(path):
    ...
    turn, plane, x, en = _np.loadtxt(
        fname, skiprows=nr_header_lines, usecols=(1, 3, 5, 7), unpack=True
    )
...
def load_dynap_ma(path):
    ...
    turn, el, pos, en = _np.loadtxt(
        fname, skiprows=nr_header_lines, usecols=(1, 2, 4, 7), unpack=True
    )

    pos = pos[::2]
    # the -abs is for cases where the mom. aperture is less than the tolerance
    accep = _np.vstack([en[1::2], -_np.abs(en[0::2])])
    n_lost = _np.vstack([turn[1::2], turn[0::2]])
    e_lost = _np.vstack([el[1::2], el[0::2]])

    return pos, accep, n_lost, e_lost
...
def track_linepass(**kwargs):
    ...
    mand_keys = _commom_keys.copy()
    mand_keys.extend(
        ["start_element", "rx0", "px0", "ry0", "py0", "de0", "dl0"]
    )
